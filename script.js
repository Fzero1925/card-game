class BlackjackGame {
  constructor() {
    ...
    this.numDecks = 6 // 使用6副牌，更接近真实赌场
    this.minCardsBeforeReshuffling = 20 // 剩余牌数少于20张时重新洗牌

    this.initializeElements()
    this.bindEvents()
    this.createDeck() // 初始化时创建牌堆
    this.updateDisplay()
  }
  ...
  needsReshuffling() {
    return this.deck.length < this.minCardsBeforeReshuffling
  }

  dealCard() {
    // 检查是否需要重新洗牌
    if (this.needsReshuffling()) {
      this.createDeck()
      this.showMessage('牌堆重新洗牌', '')
  ...
  updateBetButtons() {
    this.elements.betButtons.forEach((btn) => {
      ...
      if (
        (btn.dataset.amount === 'all' && this.currentBet === this.playerMoney) ||
  ...
  updateDisplay() {
    ...
    if (this.dealerHidden && this.dealerCards.length > 0) {
      // 只显示第一张牌的点数，A牌按11计算显示
      let visibleScore = this.dealerCards[0].value
      // 如果是A牌且会导致爆牌，显示为1
      if (this.dealerCards[0].rank === 'A' && visibleScore === 11) {
        // 在21点中，显示牌面时A通常显示为11，除非明显需要为1
        visibleScore = 11 // 保持显示为11，这是标准做法
      }
      this.elements.dealerScore.textContent = `(${visibleScore})`
    } else {
      const dealerScore = this.calculateScore(this.dealerCards)
